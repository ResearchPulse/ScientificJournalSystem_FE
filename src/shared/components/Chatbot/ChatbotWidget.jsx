...
export default function ChatbotWidget() {
  ...
  const suggestedPrompts = [
    t("common.timGiupToiTopCacBaiBaoTrongDuA"),
    t("common.danhSachTapChiQ1Nam2024"),
    t("common.baiBaoNaoCoLuotTrichDanCaoNhat"),
  ];
  ...
  const hasTable = useMemo(
    () => messages.some((item) => item.table?.data?.length),
    [messages],
  );
  ...
  return (
    <section
      className={`rp-chatbot ${isOpen ? "is-open" : ""}`}
      aria-live="polite"
    >
      {isOpen && (
        <div
          className="rp-chatbot__panel"
          role="dialog"
        ...
        >
          <header className="rp-chatbot__header">
            <div className="rp-chatbot__identity">
              ...
              <div>
                <p className="rp-chatbot__eyebrow">HyperData Lab RAG</p>
          ...
          <div className="rp-chatbot__project-bar">
            ...
            <input
              id="chatbot-project-id"
              type="number"
              min="1"
              value={projectId}
              onChange={(event) => setProjectId(event.target.value)}
              aria-label="Project ID cho chatbot"
            />
          </div>
          ...
          <div className="rp-chatbot__messages">
            {messages.map((item) => (
              <article
                key={item.id}
                className={`rp-chatbot__message rp-chatbot__message--${item.role} ${item.isError ? "is-error" : ""}`}
              >
                <div className="rp-chatbot__bubble">
                  {renderMarkdownLite(item.answer)}
                </div>
                            ...
                            >
                              {item.table.columns?.map((column) => (
                                <td
                                  ...
                                  className={
                                    numberLikeColumns.has(column.key)
                                      ? "is-number"
                                      : ""
                                  }
                                >
          ...
          >
            <textarea
              ...
              placeholder={t("common.nhapCauHoiVeBaiBaoTapChiQ1q2")}
              rows="2"
            />
            <button
      ...
      <button
        ...
        aria-label={
          isOpen ? t("common.thuNhoChatbot") : t("common.moChatbotAi")
        }
      >
