...
export default function RegionStatsPanel({
  ...
  if (!selectedCountry) {
    return <EmptyState icon="lucide:map" title={t("zone.chuaChonQuocGia")} description={t("zone.chonMotQuocGiaTuBangBenTraiDeX")} style={{
      height: '100%',
      minHeight: '420px'
    }} />;
  }
  if (error) {
    return <ErrorState message={error} onRetry={onRetry} style={{
      height: '100%',
      minHeight: '420px'
    }} />;
  }
  return <div className="p-4 journal-dark-card d-flex flex-column h-100" style={{
    minHeight: '420px'
      ...
      {/* Loading Skeleton */}
      {loading ? <div className="d-flex flex-column gap-2.5 flex-grow-1">
          <div className="skeleton-shimmer" style={{
        width: '100%',
        height: '36px',
        borderRadius: '6px'
      }} />
          {[1, 2, 3, 4, 5].map(i => <div key={i} className="skeleton-shimmer" style={{
        width: '100%',
        height: '48px',
        borderRadius: '6px'
      }} />)}
        </div> : regions.length === 0 ? <EmptyState icon="lucide:map-pin" title={t("zone.khongCoVungCon")} description={`Không tìm thấy dữ liệu phân vùng nội bộ nào cho ${selectedCountry.name} trong hệ thống.`} className="flex-grow-1 border-0 shadow-none" style={{
      minHeight: '200px'
