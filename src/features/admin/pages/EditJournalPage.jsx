...
export default function EditJournalPage() {
  ...
  const { updateJournal } = useJournalManagement();
  ...
  useEffect(() => {
    const loadDependencies = async () => {
      ...
      try {
        ...
        const { getCountryStatsApi, getRegionStatsApi } =
          await import("../../zone/api/zone.api");
        const [pubRes, countryRes, regionRes, areaRes, catRes] =
          await Promise.all([
            getPublishersApi({
              page: 1,
              limit: 100,
        ...
        const countryItems =
          countryRes.data?.data?.items || countryRes.data?.data || [];
        const regionItems =
          regionRes.data?.data?.items || regionRes.data?.data || [];
        setZones({
      ...
      try {
        ...
        if (journal) {
          setFormData({
            ...
            aimScope:
              journal.coverage || journal.description || journal.aimScope || "",
            visibility: journal.is_open_access
              ? "Public"
  ...
  const validateForm = () => {
    ...
    if (!formData.title.trim())
      newErrors.title = t("admin.tenTapChiKhongDuocDeTrong");
    if (!formData.issn.trim())
      newErrors.issn = t("admin.maSoIssnKhongDuocDeTrong");
    if (!formData.subjectCategory)
      newErrors.subjectCategory = t("admin.vuiLongChonDanhMucChinh");
    if (!formData.subjectArea.trim())
      newErrors.subjectArea = t("admin.linhVucNghienCuuCuTheKhongDuoc");
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    if (/^\d{7}[\dX]$/i.test(formattedIssn.replace(/-/g, ""))) {
      ...
      formattedIssn =
        cleanIssn.slice(0, 4) + "-" + cleanIssn.slice(4).toUpperCase();
    }

    let formattedOnlineIssn = formData.onlineIssn.trim();
    if (
      formattedOnlineIssn &&
      /^\d{7}[\dX]$/i.test(formattedOnlineIssn.replace(/-/g, ""))
    ) {
      const cleanOnline = formattedOnlineIssn.replace(/-/g, "");
      formattedOnlineIssn =
        cleanOnline.slice(0, 4) + "-" + cleanOnline.slice(4).toUpperCase();
    }

    const selectedPub = publishers.find(
      (p) => String(p.publisher_id || p.id) === String(formData.publisher),
    );
    const payload = {
      ...
      publisher_name: selectedPub
        ? selectedPub.display_name
        : "HyperData Lab Press",
    ...
    <div className="d-flex flex-column gap-3 text-start">
      <div>
        <div className="admin-breadcrumb mb-2">
          <span
            className="cursor-pointer text-muted-custom"
        ...
        <p className="text-muted-custom small mb-0">
          Modify general information, editorial settings, and visibility for
          your journal.
        </p>
      </div>
      ...
      <Form onSubmit={handleSubmit} className="mt-2">
          ...
          <Col xs={12} lg={8}>
            <Card className="border p-4 shadow-none bg-white rounded-3">
              ...
              <Form.Group className="mb-4">
                <Form.Label className="fw-medium small text-main">
                  Journal Title <span className="text-danger">*</span>
                </Form.Label>
                <Form.Control
                  type="text"
                  name="title"
                  ...
                  placeholder={t("admin.egJournalOfComputerScience")}
                />
              ...
              <Row className="g-3 mb-4">
                <Col xs={12} sm={6}>
                  <Form.Group>
                    <Form.Label className="fw-medium small text-main">
                      Print ISSN <span className="text-danger">*</span>
                    </Form.Label>
                    <Form.Control
                      type="text"
                      name="issn"
                      ...
                      placeholder={t("admin.eg15493636")}
                    />
                    <Form.Control.Feedback type="invalid">
                      {errors.issn}
                    </Form.Control.Feedback>
                ...
                <Col xs={12} sm={6}>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      onChange={handleChange}
                      placeholder={t("admin.eg1937478x")}
                    />
                  </Form.Group>
                </Col>
              </Row>
              ...
              <Form.Group className="mb-0">
                ...
                <Form.Control
                  ...
                  onChange={handleChange}
                  placeholder={t("admin.describeTheEditorialAimsScopes")}
                />
              </Form.Group>
            </Card>
          </Col>
            ...
            <div className="d-flex flex-column gap-4">
              ...
              <Card className="border p-4 shadow-none bg-white rounded-3">
                ...
                <div className="d-flex align-items-center justify-content-between py-2">
                  <div>
                    <div className="fw-bold text-main small mb-0.5">
                      Publicly Listed
                    </div>
                    <small className="text-muted-custom">
                      Visible in main index
                    </small>
                  </div>
                  <Form.Check
                    type="switch"
              ...
              <Card className="border p-4 shadow-none bg-white rounded-3">
                ...
                <Form.Group className="mb-4">
                  ...
                  >
                    {loadingDeps ? (
                      <option>{t("admin.dangTaiDanhSachNhaXuatBan")}</option>
                    ) : publishers.length === 0 ? (
                      <option value="">
                        {t("admin.khongCoNhaXuatBanNao")}
                      </option>
                    ) : (
                      publishers.map((pub) => (
                        <option
                          key={pub.publisher_id || pub.id}
                          value={pub.publisher_id || pub.id}
                        >
                          {pub.display_name}
                        </option>
                      ))
                ...
                <Form.Group className="mb-4">
                  ...
                  >
                    ...
                    {Array.from(
                      ...
                      <option
                        key={
                          cat.subject_category_id || cat.id || cat.display_name
                        }
                ...
                <Form.Group className="mb-0">
                  ...
                  >
                    ...
                    {Array.from(
                      ...
                      <option
                        key={
                          area.subject_area_id || area.id || area.display_name
                        }
        ...
        {/* Form Actions Footer */}
        <div
          className="d-flex justify-content-end gap-3 mt-4 pt-3 border-top"
          style={{
