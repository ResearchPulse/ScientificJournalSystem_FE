    ...
    getCoinPackages()
      .then((res) => {
        ...
        if (res?.success && Array.isArray(res.data)) {
          ...
          const rec = activePackages.find(
            (p) => p.coin_amount === RECOMMENDED_COIN_AMOUNT,
          );
      ...
      .catch(() => {
        if (mounted) setErrorPackages(t("wallet.loiKetNoiVuiLongThuLai"));
      })
      .finally(() => {
    ...
    <div className="topup-page topup-in-layout">
      <div className="topup-inner">
        ...
        <div className="topup-hero">
          ...
          <h1 className="topup-hero__title">
            {t("wallet.napCoinVaoTaiKhoan")}
          </h1>
          <p className="topup-hero__sub">
            {t("wallet.moKhoaNhungTiemNangNghienCuuVo")}
          </p>
        </div>
        ...
        <div className="topup-wallet-bar">
          <div className="topup-wallet-bar__left">
            ...
            <div>
              <div className="topup-wallet-bar__label">
                {t("wallet.soDuHienTai")}
              </div>
                ...
                >
                  ...
                  <div className="topup-pkg-card__top-row">
                    ...
                    <div className="topup-pkg-card__meta">
                      <div className="topup-pkg-card__label">
                        HyperData Lab Coin
                      </div>
                      <div className="topup-pkg-card__name">
                        {pkg.name.replace(/(Gói)(?=\d)/, "$1 ")}
                      </div>
                    </div>
                  </div>
                  ...
                  {pkg.bonus_coin > 0 ? (
                    ...
                    <div className="topup-pkg-card__bonus topup-pkg-card__bonus--muted">
                      ...
                      {t("wallet.khongCoBonus")}
                    </div>
                  )}

                  <div className="topup-pkg-card__footer">
                    <div className="topup-pkg-card__price">
                      {formatVND(pkg.price)}
                    </div>
        ...
        {!loadingPackages && packages.length > 0 && (
          <div className="topup-cta-row">
            ...
            >
              ...
              {t("wallet.tienHanhNapTien")}
              {selectedPkg && ` — ${formatVND(selectedPkg.price)}`}
            </button>
            <div className="topup-cta-note">
              {t("wallet.giaoDichDuocBaoMatVaMaHoa")}
              <br />
              {t("wallet.coinSeDuocCongSauKhiThanhToanT")}
            </div>
          </div>
        )}
        ...
        <div className="topup-benefits">
          <div className="topup-benefits__title">
            {t("wallet.taiSaoNenSuDungResearchpulseCo")}
          </div>
