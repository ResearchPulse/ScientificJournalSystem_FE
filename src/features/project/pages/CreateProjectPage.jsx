...
import { Icon, PrimaryButton, SearchableSelect, SearchableKeywordInput } from '@ui';
import Header from '../../landing/components/Header';
const CreateProjectPage = ({ embedded = false, onCancel }) => {
  ...
  const handleSubmit = async e => {    e.preventDefault();
    if (!title.trim() || !subjectAreaId) {
      setError(t("project.vuiLongNhapTenDuAnVaChonLinhVuc"));
      return;
    }
    setLoading(true);
    setError(null);
  ...
  const formCard = (
    <div className="glass-card p-4 p-md-5 rounded-4 shadow-sm border" style={{ background: 'var(--bg-card)' }}>
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-4">
          <label htmlFor="projectTitle" className="form-label fw-semibold text-main mb-2 small text-uppercase tracking-wider">
            {t("project.tenDuAn")}<span className="text-danger">*</span>
          </label>
          <input
            type="text"
            className="form-control form-control-lg journal-dark-input"
            id="projectTitle"
            placeholder={t("project.viDuNghienCuuUngDungDeepLearni")}
            value={title}
        ...
        <div className="mb-4">
          <label htmlFor="subjectArea" className="form-label fw-semibold text-main mb-2 small text-uppercase tracking-wider">
            {t("project.linhVucNghienCuuChinh")}<span className="text-danger">*</span>
          </label>
          <SearchableSelect
            options={areas.map(a => ({
            ...
            placeholder={t("project.chonLinhVucNghienCuu")}
        ...
        <div className="mb-5">
          ...
          {suggestedKeywords.length > 0 && (
            <div className="mt-3 small">
              ...
              {loadingSuggestions ? (
                <span className="text-muted-custom ms-2">{t("common.dangTai")}</span>
              ) : (
        ...
        <div className="d-flex gap-3 justify-content-end pt-3 border-top">
          <PrimaryButton
            ...
            variant="outline"
            className="px-4 py-2"
            onClick={() => onCancel ? onCancel() : navigate(ROUTES.PROJECTS)}
            disabled={loading}
          >
            {t("admin.huy")}
          </PrimaryButton>
  ...
  if (embedded) {
    return (
      <div className="create-project-embedded w-100" style={{ maxWidth: '720px', margin: '0 auto' }}>
        {formCard}
      </div>
    );
  }
