...
import { EmptyState, LoadingSkeleton, PrimaryButton, Icon } from '@ui';
import { Modal } from 'react-bootstrap';
import Header from '../../landing/components/Header';
import useAuth from '../../auth/hooks/useAuth';
const ProjectListPage = ({ embedded = false }) => {
  ...
  const recentProjects = recentIds
    .map(id => filteredActiveProjects.find(p => (p.project_id || p.id) === id))
    .filter(Boolean);

  const mainContent = (
    <div className={embedded ? "w-100" : "container mx-auto position-relative z-1"} style={embedded ? {} : {
      maxWidth: '1200px',
      marginTop: '40px'
    }}>
      {/* Breadcrumb - ẩn khi embedded trong Dashboard */}
      {!embedded && (
      ...
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
          <h1 className="font-display fw-bold text-main mb-2" style={{ fontSize: embedded ? '1.6rem' : '2rem' }}>
            {t("project.duAnNghienCuuCuaToi")}
          </h1>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom">
        <div className="position-relative" style={{
          ...
          <input
            ...
            placeholder={t("project.timKiemDuAn")}
            value={searchTerm}
        ...
        <div className="d-flex gap-4 text-muted-custom small">
          ...
          <span>{t("project.dangHoatDong")}{" "}<strong className="text-success">{activeProjects.length}</strong></span>
        </div>
      </div>
      ...
      {error ? (
        ...
        <EmptyState
          title={searchTerm ? "Không tìm thấy dự án phù hợp" : t("project.banChuaCoDuAnNao")}
          description={searchTerm ? "Hãy thử tìm kiếm với từ khóa khác." : t("project.taoDuAnDauTienDeBatDauTheoDoiT")}
        ...
        <div className="d-flex flex-column gap-5">
            ...
            <div>
              ...
              <div className="row g-4">
                {recentProjects.map(project => (
                  <div key={`recent-${project.project_id || project.id}`} className="col-12 col-md-6 col-lg-4 reveal-on-scroll">
          ...
          <div>
            <h5 className="font-display fw-bold text-main mb-3 d-flex align-items-center gap-2">
              ...
              <span>{t("project.tatCaDuAn")} ({filteredActiveProjects.length})</span>
            </h5>
            <div className="row g-4">
              {filteredActiveProjects.map(project => (
                <div key={project.project_id || project.id} className="col-12 col-md-6 col-lg-4 reveal-on-scroll">
  ...
  if (embedded) {
    return (
      <div className="project-list-embedded">
        {mainContent}
        {confirmModalElement}
      </div>
    );
