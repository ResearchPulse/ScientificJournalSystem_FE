...
export default function ForgotPasswordForm({ onSubmit, isLoading, apiError }) {
  ...
  return (
    <Form onSubmit={handleSubmit} noValidate>
      ...
      <InputField
        label={t("auth.diaChiEmail")}
        name="email"
        type="email"
        value={email}
        onChange={handleChange}
        onBlur={handleBlur}
        placeholder={t("auth.nameemailcom")}
        error={error}
        icon="lucide:mail"
        required
        disabled={isLoading}
      ...
      <div className="mt-4">
        <SubmitButton
          isLoading={isLoading}
          loadingText={t("auth.dangGuiYeuCau")}
          label={t("auth.guiLienKetDatLaiMatKhau")}
        />
      </div>
      ...
      <div className="text-center mt-4 text-sm font-medium d-flex align-items-center justify-content-center gap-1 flex-wrap">
        <Link
          to={ROUTES.LOGIN}
          className="auth-link d-inline-flex align-items-center gap-1.5"
        >
